...
- (BOOL) _uploadData:(NSData *)data withFileName:(NSString *)fileName toServer:(FMServer *)server error:(NSError * __autoreleasing *)error {
    BOOL success = YES;
    ...
    if (!success) {
        *error = [NSError errorWithCode:OEPErrorCodeInvalidServerURL andDescription:@"URL is invalid"];
    }
    Check(success);
    ...
    if (!success) {
        *error = [NSError errorWithCode:OEPErrorCodeFileReaderFailedInit andDescription:@"File reader failed to initialise"];
    }
    Check(success);
    ...
    if (!success) {
        *error = [NSError errorWithCode:OEPErrorCodeWriteStreamFailedInit andDescription:@"Write stream failed to initialise"];
    }
    Check(success);
    ...
    if (!success) {
        *error = [NSError errorWithCode:OEPErrorCodeWriteStreamCredentialWriteFailed andDescription:@"Failed to set username and password on write stream"];
    }
    Check(success);
    ...
    if (!success) {
        *error = [NSError errorWithCode:OEPErrorCodeUnknownFTPError andDescription:@"Unknown FTP error"];
    }
...
- (BOOL) uploadData:(NSData*)data withFileName:(NSString *)fileName toServer:(FMServer*)server error:(NSError * __autoreleasing *)error {
    if (![self _checkFMServer:server]) {
        *error = [NSError errorWithCode:OEPErrorCodeInvalidServerDetails andDescription:@"Server details invalid or incomplete"];
        return NO;
    }
    if (!data) {
        *error = [NSError errorWithCode:OEPErrorCodeNullData andDescription:@"Data to upload cannot be null"];
        return NO;
    }
    return [self _uploadData:data withFileName:fileName toServer:server error:error];
}
